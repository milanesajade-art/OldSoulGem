...
function normalizePiece(piece, index=0) {
  return {id: piece?.id || `VIDA ${String(index + 1).padStart(3,'0')}`,name: piece?.name || 'Untitled Piece',status: piece?.status || 'Designer Review',visibility: piece?.visibility === 'hidden' ? 'hidden' : 'public',price: piece?.price || 'Private',materials: piece?.materials || '14K yellow gold',story: piece?.story || '',image: normalizeImageUrl(piece?.image || '')};
}
...
function flash(selector, message) { const el = $(selector); if (!el) return; el.textContent = message; setTimeout(() => { if (el.textContent === message) el.textContent = ''; }, 4500); }
function nextId() { const max = Math.max(0, ...workspace.pieces.map((p) => parseInt(String(p.id).replace(/\D/g,''),10) || 0)); return `VIDA ${String(max + 1).padStart(3,'0')}`; }
function activePiece() { return workspace.pieces.find((p) => p.id === active) || workspace.pieces[0] || null; }
function renderStats() { $('#statPieces').textContent = workspace.pieces.length; $('#statPublic').textContent = workspace.pieces.filter((p) => p.visibility === 'public').length; $('#statHidden').textContent = workspace.pieces.filter((p) => p.visibility === 'hidden').length; $('#statInquiries').textContent = (readJson(INQUIRY_KEY) || []).length; }
function renderList() {
...
function populateImageSelect(value='') {
  const select = $('#pieceImage'); const libraryValue = MEDIA_ASSETS.includes(value) ? value : '';
  select.innerHTML = MEDIA_ASSETS.map((a) => `<option value="${esc(a)}" ${a === libraryValue ? 'selected' : ''}>${a ? esc(MEDIA_LABELS[a] || a) : 'No library image'}</option>`).join('');
}
function validatePiece(p) { const issues = []; if (!p.name.trim()) issues.push('Name is required.'); if (!p.materials.trim()) issues.push('Materials are required.'); if (!p.story.trim()) issues.push('Story is required.'); if (!p.price.trim()) issues.push('Price or availability is required.'); if (p.visibility === 'public' && !p.image && !['VIDA 001','VIDA 002'].includes(p.id)) issues.push('Public piece has no image; storefront will use a branded placeholder.'); return issues; }
function renderValidation(p) { const issues = validatePiece(p); $('#validationBox').innerHTML = issues.length ? `<strong>Review</strong><ul>${issues.map((i) => `<li>${esc(i)}</li>`).join('')}</ul>` : '<strong class="ok">Ready to preview.</strong>'; }
function renderEditor() { const p = activePiece(); if (!p) return; $('#pieceId').value = p.id; $('#pieceName').value = p.name; $('#pieceStatus').value = p.status; $('#pieceVisibility').value = p.visibility; $('#piecePrice').value = p.price; $('#pieceMaterials').value = p.materials; populateImageSelect(p.image); $('#pieceImageUrl').value = p.image && !MEDIA_ASSETS.includes(p.image) ? p.image : ''; $('#pieceStory').value = p.story; $('#editorTitle').textContent = p.name; $('#editorMeta').textContent = `${p.id} • ${p.visibility === 'hidden' ? 'Hidden' : p.status}`; renderValidation(p); }
function syncPieceForm() { const p = activePiece(); if (!p) return null; p.name = $('#pieceName').value.trim(); p.status = $('#pieceStatus').value; p.visibility = $('#pieceVisibility').value; p.price = $('#piecePrice').value.trim(); p.materials = $('#pieceMaterials').value.trim(); const external = normalizeImageUrl($('#pieceImageUrl').value); p.image = external || $('#pieceImage').value; if (external) $('#pieceImageUrl').value = external; p.story = $('#pieceStory').value.trim(); return p; }
function renderSiteEditor() { SITE_FIELDS.forEach((key) => { const input = $(`[data-site-key="${key}"]`); if (input) input.value = workspace.site[key] || ''; }); }
function syncSiteForm() { SITE_FIELDS.forEach((key) => { const input = $(`[data-site-key="${key}"]`); if (input) workspace.site[key] = input.value.trim(); }); }
function renderMedia() {
  ...
  $$('.creator-media-card img').forEach((img) => img.addEventListener('error', () => { img.alt = 'Image unavailable'; }, {once:true}));
...
function renderInquiries() {
  ...
  $('#inquiryRows').innerHTML = inquiries.length ? inquiries.map((i, idx) => `<tr><td>${esc(i.name || '—')}</td><td>${esc(i.interest || '—')}</td><td>${esc(i.email || '—')}</td><td><select class="inquiry-status" data-index="${idx}"><option ${i.status === 'Preview' ? 'selected' : ''}>Preview</option><option ${i.status === 'Contacted' ? 'selected' : ''}>Contacted</option><option ${i.status === 'Appointment' ? 'selected' : ''}>Appointment</option><option ${i.status === 'Closed' ? 'selected' : ''}>Closed</option></select></td><td>${i.createdAt ? new Date(i.createdAt).toLocaleDateString() : '—'}</td></tr>`).join('') : '<tr><td colspan="5" class="creator-muted">No saved preview inquiries yet. Live customer inquiries are emailed directly to Alé.</td></tr>';
...
function writePreview() { syncPieceForm(); syncSiteForm(); saveWorkspace(); const payload = { version: 2, publishedAt: new Date().toISOString(), pieces: clone(workspace.pieces), site: clone(workspace.site) }; localStorage.setItem(PREVIEW_KEY, JSON.stringify(payload)); return payload; }
...
function exportWorkspace() { syncPieceForm(); syncSiteForm(); saveWorkspace(); const blob = new Blob([JSON.stringify({exportedAt:new Date().toISOString(), ...workspace}, null, 2)], {type:'application/json'}); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'vida-creator-workspace.json'; a.click(); URL.revokeObjectURL(url); }
function renderReview() { const preview = readJson(PREVIEW_KEY); $('#previewState').textContent = preview ? 'ACTIVE' : 'CLEAN'; $('#previewSummary').textContent = preview ? `Preview saved ${new Date(preview.publishedAt).toLocaleString()}. It affects this browser only.` : 'No Creator preview is active. The storefront is showing the repository baseline.'; }
function addPiece() { const id = nextId(); workspace.pieces.push(normalizePiece({id, name:'Untitled Piece', status:'Designer Review', visibility:'hidden', price:'Private', materials:'14K yellow gold', story:'', image:''}, workspace.pieces.length)); active = id; saveWorkspace(); renderAll(); flash('#saveNote',`${id} created hidden by default.`); }
...
function activateView(view) { $$('.creator-nav button[data-view]').forEach((b) => b.classList.toggle('active', b.dataset.view === view)); $$('.creator-view').forEach((v) => v.classList.toggle('active', v.id === `view-${view}`)); location.hash = view; if (view === 'media') renderMedia(); if (view === 'inquiries') renderInquiries(); if (view === 'review') renderReview(); }
function bind() {
...
document.addEventListener('DOMContentLoaded', () => { renderAll(); bind(); const requested = location.hash.replace('#',''); activateView(['collection','site','media','inquiries','review'].includes(requested) ? requested : 'collection'); });
